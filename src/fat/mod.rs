use std::{
    ...
    mem::size_of, collections::HashSet,
};
...
impl FAT {
    ...
    fn read_sector(&mut self, sector: u64) -> Option<[u8; 512]> {
        let mut buf = [0; 512];
        self.file
            .seek(SeekFrom::Start(self.sector_to_byte(sector))).ok()?;
    ...
    pub fn mkdir(&mut self, path: &str) -> Result<(), FATError> {
        ...
        let mut new_entry = Entry::new(filename, 0, 0, Flags::Occupied as u32 | Flags::Directory as u32).ok_or(FATError::FilenameTooLong)?;
            
        let mut current_cluster = entry.cluster();
            
        while current_cluster != Self::mark_read_done() {
            let mut dirents = self.read_cluster_entries(current_cluster).ok_or(FATError::CannotRead)?;
            for dirent in dirents.iter_mut() {
                if dirent.flags() & Flags::Occupied as u32 == 0 {
                    ...
                    self.write_cluster(cluster, FAT::empty_cluster()[0..4096].try_into().unwrap()).ok_or(FATError::CannotWrite)?;
                    let mut entries = self.read_cluster_entries(cluster).ok_or(FATError::CannotRead)?;
                        
                    entries[0] = Entry::new(".", 0, new_entry.cluster(), Flags::Occupied as u32 | Flags::Directory as u32 | Flags::System as u32).unwrap();
                    entries[1] = Entry::new("..", 0, entry.cluster(), Flags::Occupied as u32 | Flags::Directory as u32 | Flags::System as u32).unwrap();
                        
                    self.write_cluster_entries(cluster, &entries).ok_or(FATError::CannotWrite)?;
                        
                    *dirent = new_entry;
                    self.write_cluster_entries(current_cluster, &dirents).ok_or(FATError::CannotWrite)?;
                    return Ok(());
                }
            }
                
            current_cluster = self.next_cluster(current_cluster).ok_or(FATError::CannotRead)?;
    ...
    pub fn new_file<T: Read + Seek>(&mut self, path: &str, mut infile: T) -> Result<(), FATError> {
        let file_size = infile.seek(SeekFrom::End(0)).map_err(|_| FATError::CannotRead)?;
        ...
        let mut new_entry = Entry::new(filename, file_size as u32, 0, Flags::Occupied as u32).ok_or(FATError::FilenameTooLong)?;
        ...
        while current_cluster != Self::mark_read_done() {
            ...
            for dirent in dirents.iter_mut() {
                if dirent.flags() & Flags::Occupied as u32 == 0 {
                    let cluster_size = (self.header.as_ref().unwrap().sectors_per_cluster() * self.header.as_ref().unwrap().bytes_per_sector()) as u64;
                    ...
                    loop {
                        ...
                        self.write_cluster(cluster, buffer[..].try_into().unwrap()).ok_or(FATError::CannotWrite)?;
    ...
    pub fn cat<T: Write>(&mut self, path: &str, mut outfile: T) -> Result<(), FATError> {
        ...
        while cluster != Self::mark_read_done() {
            ...
            outfile.write(&bytes[0..limit as usize]).map_err(|_| FATError::CannotWrite)?;
    ...
    pub fn info(&mut self, path: &str) -> Result<(), FATError> {
        ...
        println!("{} {}", entry.name(), clusters.iter().map(|n| n.to_string()).collect::<Vec<_>>().join(", "));
        Ok(())
    }
    ...
    fn remove(&mut self, path: &str, flags: u32) -> Result<(), FATError> {
        ...
        while current_cluster != Self::mark_read_done() {
            ...
            let mut entries = self.read_cluster_entries(current_cluster).ok_or(FATError::CannotRead)?;
            
            for entry in entries.iter_mut() {
                if entry.name() == filename && entry.flags() == flags {
                    if flags & Flags::Directory as u32 == Flags::Directory as u32 && !self.is_empty(entry)? {
    ...
    pub fn move_file(&mut self, source: &str, dest: &str) -> Result<(), FATError> {
        ...
        let mut src_cluster = dir_src.cluster();
        // let mut entries: Vec<Entry>;
        let mut entry: Entry;
        
        'outer: loop {
            let mut entries = self.read_cluster_entries(src_cluster).ok_or(FATError::CannotRead)?;
            
            for mut _entry in entries.iter_mut() {
                if _entry.name() == file1 && _entry.flags() & (Flags::Occupied as u32 | Flags::Directory as u32 | Flags::System as u32) == Flags::Occupied as u32 {
                    entry = _entry.clone();
                    _entry.set_flags(0);
                    self.write_cluster_entries(src_cluster, &entries);
                    break 'outer;
                }
            }
            
            src_cluster = self.next_cluster(src_cluster).ok_or(FATError::CannotRead)?;
            if src_cluster == Self::mark_bad_cluster() {
                return Err(FATError::CannotRead);
            }
        };
        
        let mut dst_cluster = dir_dest.cluster();
        while dst_cluster != Self::mark_read_done() {
            let mut dst_entries= self.read_cluster_entries(src_cluster).ok_or(FATError::CannotRead)?;
            for mut _entry in dst_entries.iter_mut() {
                if _entry.flags() & Flags::Occupied as u32 == 0 {
                    entry.set_name(file2);
                    *_entry = entry;
                    self.write_cluster_entries(dst_cluster, &dst_entries);
                    return Ok(());
                }
            }
            
            dst_cluster = self.next_cluster(src_cluster).ok_or(FATError::CannotRead)?;
            if dst_cluster == Self::mark_bad_cluster() {
                return Err(FATError::CannotRead);
            }
        }
    ...
    pub fn copy(&mut self, source: &str, dest: &str) -> Result<(), FATError> {
        ...
        let cluster_size = self.header.as_ref().unwrap().sectors_per_cluster() * self.header.as_ref().unwrap().bytes_per_sector();
        ...
        let mut new_entry = Entry::new(filename, entry.size(), 0, Flags::Occupied as u32).ok_or(FATError::FilenameTooLong)?;
        ...
        while cluster != Self::mark_read_done() {
            ...
            for dirent in entries.iter_mut() {
                if dirent.flags() & Flags::Occupied as u32 == 0 {
                    let alloc = self.allocate_clusters(cluster_count).map_err(|_| FATError::CannotRead)?;
                    ...
                    *dirent = new_entry;
                    ...
                    while cluster_a != Self::mark_read_done() || cluster_b != Self::mark_read_done() {
                        ...
                        self.write_cluster(cluster_a, cluster).ok_or(FATError::CannotWrite)?;
                    ...
                    self.write_cluster_entries(cluster, &entries).ok_or(FATError::CannotRead)?;
        ...
        while cluster != Self::mark_read_done() {
            ...
            if entry.flags() & Flags::Directory as u32 == Flags::Directory as u32 {
                ...
                for dirent in entries {
                    if dirent.flags() & Flags::Occupied as u32 == Flags::Occupied as u32 && dirent.name() != "." && dirent.name() != ".." {
    ...
    fn write_header(&mut self) -> Option<()> {
        ...
        let bytes = [
            header.bytes_per_sector().to_le_bytes(),
            header.sectors_per_cluster().to_le_bytes(),
            header.sector_count().to_le_bytes(),
            header.fat_count().to_le_bytes(),
        ...
        self.file.write_vectored(&io_slice).ok()?;
        let cluster_count = header.sector_count() / header.sectors_per_cluster();

        let fat_sectors = 1 + size_of::<u32>() as u32 * cluster_count / header.bytes_per_sector();

        self.file
            .seek(SeekFrom::Start(header.bytes_per_sector() as u64)).ok()?;
        for _ in 0..header.sector_count() - 1 {
            self.file
                .write(&FAT::empty_cluster()[0..header.bytes_per_sector() as usize]).ok()?;
        }

        self.file
            .seek(SeekFrom::Start(header.bytes_per_sector() as u64)).ok()?;
        self.file.write(&FAT::mark_bad_cluster().to_le_bytes()).ok()?;
